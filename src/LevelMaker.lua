...
function LevelMaker.createMap(level)
    ...
    -- randomly generate key powerup
    --local keyFlag = math.random(0, 1)
    local keyFlag = 1
    local keyTier = 3
    local keyColor = 6
    -- select random quad
    local rand_row = math.random(0, numRows)
    local rand_col = math.random(0, numCols)
    ...
    for y = 1, numRows do
        ...
        for x = 1, numCols do
            ...
            --if keyFlag is set to true, generate the key quad on the appropriate location
            if keyFlag == 1 then
                if x == rand_col and y == rand_row then
                    b.color = keyColor
                    b.tier = keyTier
                end
            end
